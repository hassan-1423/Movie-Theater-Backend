...
import { Movie, MovieCategory } from "./movieSchema"
...
export const MovieController: Record<string, RouteShorthandOptionsWithHandler> =
  {
    createMovie: {
    ...
    movieCategory: {
      ...
      handler: async (req) => {
        const body = req.body as Movie
        return await MovieService.createMovieCategory(body)
      },
    },
    getmovieCategory: {
      handler: async (req) => {
        const body = req.params
        return await MovieService.getMovieCategories(body)
      },
    },
    updateMovieCategory: {
      preValidation: [validateToken, hasRole(UserRole.ADMIN)],
      ...
      handler: async (req) => {
        ...
        console.log(body)
        const id = params.id
        return await MovieService.updateMovieCategory(id, body)
      },
    },
  }
